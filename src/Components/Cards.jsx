import React from 'react'
import CardUI from "./CardUI";


const Cards = () => {
    return (
        <div className="team">
             <div className="container mt-5 pt-5 mb-4">
             <h2 className="text-primary text-center mb-3">Our Team</h2>
                <div className="row  d-flex align-items-center pl-2 text-center" style={{justifyContent:"space-between"}}>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Asish.jpg" title="V Asish Raju" position="Lead"
                            twitter="https://twitter.com/asish_io"
                            github="https://github.com/AsishRaju"
                            linkedin="https://www.linkedin.com/in/asishraju/"
                            insta="https://www.instagram.com/asish.io/"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Aditya.jpg" title="Aditya NVS" position="Operational Lead"
                            twitter="www.twitter.com/theadityanvs"
                            github="www.github.com/theadityanvs"
                            linkedin="www.linkedin.com/in/theadityanvs"
                            insta="www.instagram.com/Aditya.nvs"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Biswajit.jpg" title="Biswajit Sarangi" position="Operational Lead"
                            twitter="https://twitter.com/Biswaji39491247"
                            github="https://github.com/Biswa5812"
                            linkedin="https://www.linkedin.com/in/biswajitsarangi/"
                            insta="https://www.instagram.com/biswasarangi/"
                        />
                    </div>
                </div>

                <div className="row  d-flex align-items-center pl-2 mt-3" style={{justifyContent:"space-between"}}>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Sreeram.jpg" title="T Sreeram" position="Web Developer"
                            twitter="https://twitter.com/SreeramThatava1?s=08"
                            github="https://github.com/SreeramThatavarthi/"
                            linkedin="https://www.linkedin.com/in/sreeram-thatavarthi"
                            insta="https://www.instagram.com/ram_thatavarthi/"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Naveen.jpg" title="J V Naveen Babu" position="Web Developer"
                            twitter="https://twitter.com/JVNaveenBabu1"
                            github="https://github.com/jvnaveenbabu"
                            linkedin="https://www.linkedin.com/in/j-v-naveen-babu/"
                            insta="https://www.instagram.com/_.naveen_.09/"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/dummy-men.jpg" title="Dheeraj Chowdary" position="Django Developer"
                            twitter=""
                            github=""
                            linkedin=""
                            insta=""
                        />
                    </div>
                </div>

                <div className="row  d-flex align-items-center pl-2 mt-3" style={{justifyContent:"space-between"}}>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Sunil.jpg" title="V Sunil Tagore" position="ML/DS"
                            twitter="https://twitter.com/vsuniltagore858"
                            github="https://github.com/Sunil-tagore"
                            linkedin="https://www.linkedin.com/in/sunil-tagore-veluturi-366b8917b/"
                            insta="https://www.instagram.com/vsuniltagore/"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Sai.jpg" title="M Sai Sashank" position="ML/DS"
                            twitter="https://twitter.com/KrishnaSashank2"
                            github="https://github.com/k-sashank"
                            linkedin="https://www.linkedin.com/in/krishna-sashank-madipally-513194182/"
                            insta="https://www.instagram.com/kris_sashank/"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Charan.jpg" title="Charan Kumar" position="UI/UX Designer"
                            twitter=""
                            github="https://github.com/CharanVenkatRajKumar"
                            linkedin="https://www.linkedin.com/in/charan-venkat-raj-kumar-7254591b6/"
                            insta="https://www.instagram.com/charan_26/"
                        />
                    </div>
                </div>

                <div className="row  d-flex align-items-center pl-2 mt-3" style={{justifyContent:"space-between"}}>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/dummy-men.jpg" title="Anish Dora" position="UI/UX Designer"
                            twitter=""
                            github=""
                            linkedin=""
                            insta=""
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Jyoti.jpg" title="Jyotirmayi Swain" position="UI/UX Designer"
                            twitter=""
                            github=""
                            linkedin="https://www.linkedin.com/in/jyotirmayi-swain-02b1b71a0/"
                            insta=""
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/dummy-img.jpg" title="Sheetal Patnaik" position="Content Writer"
                            twitter=""
                            github=""
                            linkedin=""
                            insta=""
                        />
                    </div>
                </div>

                <div className="row  d-flex align-items-center pl-2 mt-3" style={{justifyContent:"space-between"}}>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Sanjana.jpg" title="Sanjana Rayavarapu" position="Content Writer"
                            twitter=""
                            github="https://github.com/Sanju271"
                            linkedin="https://www.linkedin.com/in/sanjana~rayavarapu/"
                            insta=" https://www.instagram.com/sanjana_rayavarapu/"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/dummy-men.jpg" title="A Neeraj" position="Mobile App Developer"
                            twitter=""
                            github="https://github.com/A-Neeraj "
                            linkedin="https://www.linkedin.com/in/neeraj-a-23947617b/"
                            insta=""
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Divyansh.jpg" title="Divyansh Gupta" position="Mobile App Developer"
                            twitter="https://twitter.com/divyansh_gupta7"
                            github="https://github.com/divyansh0210/anshify"
                            linkedin="https://www.linkedin.com/in/divyansh-gupta-935637197/"
                            insta="https://www.instagram.com/divyaaaansh_0210/"
                        />
                    </div>
                </div>

                <div className="row  d-flex align-items-center pl-2 mt-3" style={{justifyContent:"space-between"}}>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Rakesh.jpg" title="Rakesh Naidu" position="Competitive Programming"
                            twitter="https://twitter.com/Rakesh_2345"
                            github="https://github.com/Rakesh-Naidu"
                            linkedin="https://www.linkedin.com/in/rakesh-naidu-guntreddi/"
                            insta="https://www.instagram.com/rakeshguntreddi/"
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Atul.jpg" title="Atul Dwivedi" position="Competitive Programming"
                            twitter=""
                            github=""
                            linkedin=""
                            insta=""
                        />
                    </div>
                    <div className="col-md-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Kushal.jpg" title="kushal Ippili" position="Competitive Programming"
                            twitter="https://twitter.com/kushalippili"
                            github="https://github.com/kushal13112001"
                            linkedin="https://www.linkedin.com/in/kushal-ippili-abb810190"
                            insta="https://www.instagram.com/kushal_ippili/"
                        />
                    </div>
                </div>

                <div className="row  d-flex align-items-center pl-2 mt-3" style={{justifyContent:"center"}}>
                    <div className="col-lg-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Guru.jpg" title="Guru Charan" position="Competitive Programming"
                            twitter="https://twitter.com/GuruCha93995977"
                            github="https://github.com/guruc-134"
                            linkedin="https://www.linkedin.com/in/guru-charan-7103561a1/"
                            insta="https://www.instagram.com/gurucharan13/"
                        />
                    </div>
                    <div className="col-lg-4 col-sm-12 pr-0 mr-0">
                        <CardUI src="/Images/Bidisha.jpg" title="Bidisha" position="PR and Outreach"
                            twitter=""
                            github="https://github.com/Bidisha28"
                            linkedin="http://www.linkedin.com/in/bidisha-mukherjee-20859a1b6"
                            insta="https://instagram.com/bidisha.mukherjee_?igshid=17u2uj2idyl2g"
                        />
                    </div>
                </div>

            </div>
        </div>
           
    )
}

export default Cards;